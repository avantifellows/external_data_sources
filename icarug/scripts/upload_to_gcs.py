#!/usr/bin/env python3
"""
Upload ICAR-UG cutoffs data to GCS.

Uploads (mirrors the moe/ convention):
  - Raw:   the official PDF and its CSV extraction, as-is
           gs://avantifellows-external-data/icarug/raw/<file>
           (traceability only; not loaded to BQ)
  - Clean: the parsed fact → parquet
           gs://avantifellows-external-data/icarug/clean/icarug_fact_cutoffs.parquet
           (this is what load_bq.py loads)

Run build_clean.py first to produce clean/icarug_fact_cutoffs.parquet.
...
def upload_raw(client, dry_run: bool) -> None:
    ...
    for rf in RAW_FILES:
        if not rf.local_path.exists():
            raise SystemExit(f"missing raw file: {rf.local_path}")
        ...
        client.bucket(GCS_BUCKET).blob(rf.gcs_path).upload_from_filename(
            str(rf.local_path), content_type=("application/pdf" if rf.name.endswith(".pdf") else "text/csv")
        )
        print(f"  ✓ {msg}")
...
def main() -> None:
    ...
    group.add_argument("--raw-only", action="store_true", help="Upload only the raw files")
