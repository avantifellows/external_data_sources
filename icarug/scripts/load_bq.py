#!/usr/bin/env python3
"""
Load ICAR-UG cutoffs clean parquet from GCS into BigQuery.
...
Pre-reqs (one-time):
  ...
  (Parquet must already be in gs://avantifellows-external-data/icarug/clean/
...
def main() -> None:
    ...
    print(f"icarug → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in TABLES:
