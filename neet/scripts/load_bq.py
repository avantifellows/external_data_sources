...
Pre-reqs (one-time):
  ...
  (Parquet must already be in gs://avantifellows-external-data/neet/clean/
   — see scripts/upload_to_gcs.py.)

Usage:
  python3 scripts/load_bq.py                            # load all tables
  python3 scripts/load_bq.py --table neet_dim_marks_matrix_2026
...
def main() -> None:
    ...
    print(f"neet → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in chosen:
