...
> **Status inside `neet/`:** this family's extracts feed the `neet_*` tables (the 2026 matrix uses them for ~10 states, and they cross-validate `neet_fact_cutoffs`). The standalone `state_medical_fact_closing_ranks` table is **deliberately not loaded** — it would duplicate `neet_fact_cutoffs`. Do not run `scripts/load_bq.py` here; the canonical loaders live in `neet/scripts/`.
