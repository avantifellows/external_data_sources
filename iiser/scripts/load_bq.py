#!/usr/bin/env python3
"""
Load IISER closing ranks clean parquet from GCS into BigQuery.
...
Pre-reqs (one-time):
  ...
  (Parquet must already be in gs://avantifellows-external-data/iiser/clean/
...
def main() -> None:
    ...
    print(f"iiser → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in TABLES:
