#!/usr/bin/env python3
"""
Upload AIIMS B.Sc. Nursing allotments data to GCS.

Uploads (mirrors the moe/ convention):
  - Raw:   the source PDF, as-is (they ARE the raw artifact)
           gs://avantifellows-external-data/aiimsnursing/raw/<pdf>
           (traceability only; not loaded to BQ)
  - Clean: the parsed fact → parquet
           gs://avantifellows-external-data/aiimsnursing/clean/aiimsnursing_fact_cutoffs.parquet
           (this is what load_bq.py loads)

Run build_clean.py first to produce clean/aiimsnursing_fact_cutoffs.parquet.
