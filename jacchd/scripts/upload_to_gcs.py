#!/usr/bin/env python3
"""
Upload JAC Chandigarh cutoffs data to GCS.

Uploads (mirrors the moe/ convention):
  - Raw:   the source sheet export (CSV), as-is
           gs://avantifellows-external-data/jacchd/raw/<csv>
           (traceability only; not loaded to BQ)
  - Clean: the parsed fact → parquet
           gs://avantifellows-external-data/jacchd/clean/jacchd_fact_cutoffs.parquet
           (this is what load_bq.py loads)

Run build_clean.py first to produce clean/jacchd_fact_cutoffs.parquet.
...
def upload_raw(client, dry_run: bool) -> None:
    ...
    for rf in RAW_FILES:
        if not rf.local_path.exists():
            raise SystemExit(f"missing raw sheet: {rf.local_path}")
        ...
        client.bucket(GCS_BUCKET).blob(rf.gcs_path).upload_from_filename(
            str(rf.local_path), content_type="text/csv"
        )
        print(f"  ✓ {msg}")
