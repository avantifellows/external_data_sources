#!/usr/bin/env python3
"""
Load UPTAC OR-CR clean parquet from GCS into BigQuery.
...
Pre-reqs (one-time):
  ...
  (Parquet must already be in gs://avantifellows-external-data/uptac/clean/
...
def main() -> None:
    ...
    print(f"uptac → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in TABLES:
