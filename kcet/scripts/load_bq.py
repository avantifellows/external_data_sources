"""
Load KCET clean parquet from GCS into BigQuery (WRITE_TRUNCATE).

Pre-reqs:
  - parquet staged at gs://avantifellows-external-data/kcet/clean/
...
def main() -> None:
    ...
    print(f"KCET → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in TABLES:
