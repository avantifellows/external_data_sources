...
| CMS-E (India) | [`nss-comprehensive-modular-survey/`](nss-comprehensive-modular-survey/) | Active | Comprehensive Modular Survey: Education (MoSPI, NSS 80th Round, Apr–Jun 2025) — household expenditure on **school education only** (pre-primary → Class XII), by state, gender, class, school type and social group, with private-coaching spend itemised separately. 52,085 households / 59,417 students, weighted to 242.3m students. Unit-level CSV → parse → GCS → BQ. → 2 tables: `cmse_fact_student`, `cmse_fact_household`. **Read [`schemas/README.md`](nss-comprehensive-modular-survey/schemas/README.md) first — four documented traps (government in-kind valued at zero, integrated coaching invisible to the coaching columns, state fee regulation confounding rupee thresholds, hostel single-member households inverting per-capita ranks) change results materially.** |
