#!/usr/bin/env python3
"""
Upload BHU UG cutoffs data to GCS.

Uploads (mirrors the moe/ convention):
  - Raw:   the official PDFs and their CSV extraction, as-is
           gs://avantifellows-external-data/bhuug/raw/<file>
           (traceability only; not loaded to BQ)
  - Clean: the parsed fact → parquet
           gs://avantifellows-external-data/bhuug/clean/bhuug_fact_cutoffs.parquet
           (this is what load_bq.py loads)

Run build_clean.py first to produce clean/bhuug_fact_cutoffs.parquet.
