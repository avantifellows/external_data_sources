...
every branch/programme string in our 15 cutoff sources to it. This is the
join that lets "CSE" mean one thing across JoSAA, KCET, WBJEE, OJEE,
MHT-CET, the EAPCETs, GUJCET, TNEA, KEAM, CLAT, NEET, JAC Chandigarh, AIIMS B.Sc. Nursing and the fees sheet —
...
- **`external_data_sources.exam_branch_mapping`** (1,614 rows) — one row
...
(`evidence/branches_from_cutoff_tables.csv`) is regenerated by pulling
distinct branch columns from each `*_fact_cutoffs` table — see the
branch-mapping notes in `evidence/branches_to_map_notes.md` for the
