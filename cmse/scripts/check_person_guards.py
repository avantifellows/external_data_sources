...
def build_all():
    ...
    return per, students, C.build_person_roster(per, ctx, states), households


def main() -> None:
    per, students, persons, households = build_all()
    asleep = []
    ...
    expect_fail("age gate catches a school-age member who was never asked",
                lambda: C.assert_person_grain(persons, bad_per, students, households))
    ...
    expect_fail("row-set anchor catches a swapped person at an IDENTICAL row count",
                lambda: C.assert_person_grain(swapped, per, students, households))
    ...
    expect_fail("weight anchor catches a drifted multiplier",
                lambda: C.assert_person_grain(reweighted, per, students, households))
    ...
    expect_fail("grain catches a duplicated (household_id, person_serial_no)",
                lambda: C.assert_person_grain(duped, per, students, households))
    ...
    expect_fail("band check catches a person in no age band",
                lambda: C.assert_person_grain(unbanded, per, students, households))
    ...
    C.assert_person_grain(persons, per, students, households)
