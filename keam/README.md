...
into one fact; architecture and B.Pharm PDFs archived verbatim for later
parsing. Kerala's NEET-based medical rounds (MBBS/allied) live with the
NEET source: neet/raw/kerala_cee_2025_*_lastranks.pdf. The Kerala counterpart to `wbjee/`, `tnea/`, `kcet/`.

## Pipeline shape

```
keam/raw/KEAM_<year>_<stream>_<phase>.pdf     12 PDFs + the 2 listing pages
...
- **BigQuery**: `external_data_sources.keam_fact_cutoffs` (schema doc in
  data-assistant, regenerated CLAUDE.md row).
- **College predictor**: the KEAM exam serves 2026 P1/P2 buckets (Trial
  excluded), generated by the predictor repo's `scripts/build_keam_2026.py`.
- **Open data** (`/datasets`): the 12 KEAM-stream PDFs + the extracted engineering CSV; the medical PDFs publish under the NEET-UG dataset.
