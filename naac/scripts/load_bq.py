"""
Load NAAC parquet files from GCS into BigQuery.
...
Pre-reqs (one-time):
  ...
  (Parquet files must already be in gs://avantifellows-external-data/naac/
...
def main() -> None:
    ...
    print(f"NAAC → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in chosen:
