"""
Load GUJCET clean parquet from GCS into BigQuery (WRITE_TRUNCATE).

Pre-reqs:
  - parquet staged at gs://avantifellows-external-data/gujcet/clean/
...
def main() -> None:
    ...
    print(f"GUJCET → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in TABLES:
