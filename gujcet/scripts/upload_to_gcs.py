"""Upload auditable GUJCET raw sources and deterministic clean Parquet to GCS."""
...
def _pdf_items() -> list[tuple[Path, str]]:
    ...
    The auditable source of record: every number in the fact table traces back
    to one of these pages. Gitignored — they ride in GCS, never in the repo.
    Flat (2 files), unlike mhtcet's per-stream folders.
    """
...
def _items(include_raw: bool, include_clean: bool, include_pdfs: bool) -> list[tuple[Path, str]]:
    ...
    if include_pdfs:
        ...
        if not pdfs:
            raise SystemExit(
                f"No PDFs found under {RAW_PDF_DIR}. Copy the two ACPC closure "
                "PDFs from futures-v2 state_cet/scrape/source/GJ/*/ into raw/pdfs/."
            )
        items += pdfs
    if include_clean:
        for table in TABLES:
