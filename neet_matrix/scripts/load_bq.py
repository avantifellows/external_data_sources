#!/usr/bin/env python3
"""
Load NEET 2026 marks-matrix parquet from GCS into BigQuery.
...
Pre-reqs (one-time):
  ...
  (Parquet must already be in gs://avantifellows-external-data/neet_matrix/clean/
   — see scripts/upload_to_gcs.py.)

Usage:
  python3 scripts/load_bq.py                            # load all tables
  python3 scripts/load_bq.py --table neet_matrix_fact_mbbs_seats
...
def main() -> None:
    ...
    print(f"neet_matrix → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in chosen:
